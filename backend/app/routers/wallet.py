...
WEB3_PROVIDER = os.getenv("WEB3_PROVIDER_URL") or os.getenv("WEB3_PROVIDER") or "http://127.0.0.1:8545"
...
def verify_signature(payload: VerifySignatureRequest, response: Response, db: Session = Depends(get_db)):
    ws = WalletService(db, WEB3_PROVIDER)
...
def link_wallet(payload: VerifySignatureRequest, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ws = WalletService(db, WEB3_PROVIDER)
    addr = Web3.to_checksum_address(payload.wallet_address)
...
def refresh_balance(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="RPC unavailable")
        
    if not success:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Could not fetch balance")
