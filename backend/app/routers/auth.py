...
from ..schemas.auth import (
    ...
    LoginResponse, TwoFactorLoginRequest, RefreshTokenResponse,
    EmailVerificationRequest, EmailVerificationResponse,
    ResendVerificationEmailRequest, ResendVerificationEmailResponse,
    VerificationStatusResponse, ForgotPasswordRequest, ForgotPasswordResponse,
    ResetPasswordRequest, ResetPasswordResponse, TwoFactorSetupResponse,
...
from ..models import CreditHistory, EmailVerificationToken, PasswordResetToken, User, RefreshToken, UserRole
...
async def verify_email(req: EmailVerificationRequest, response: Response, db: Session = Depends(get_db)):
    service = AuthService(db)
...
async def resend_verification(req: ResendVerificationEmailRequest, db: Session = Depends(get_db)):
    service = AuthService(db)
    user, token = service.prepare_resend_verification(req.email)
    
    if not user:
        return {"message": "If registered, a verification link has been sent.", "email": req.email}
    
    if not token:
        return {"message": "This email is already verified.", "email": req.email}

    if email_service.send_verification_email(user.email, token):
        return {"message": "Verification email sent. Check your inbox.", "email": req.email}
...
async def login_2fa(payload: TwoFactorLoginRequest, response: Response, db: Session = Depends(get_db)):
    service = AuthService(db)
...
def enable_2fa(payload: TwoFactorEnableRequest, access_token: str | None = Cookie(None), db: Session = Depends(get_db)):
    user = _get_user_from_cookie(access_token, db)
...
def disable_2fa(payload: TwoFactorDisableRequest, access_token: str | None = Cookie(None), db: Session = Depends(get_db)):
    user = _get_user_from_cookie(access_token, db)
