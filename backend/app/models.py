...
class User(Base):
    ...
    # Relationships
    connected_wallets = relationship(
        "LinkedWallet", back_populates="user", cascade="all, delete-orphan"
    )
    invoices = relationship(
        "Invoice", back_populates="seller", foreign_keys="Invoice.seller_id"
    )
    credit_history = relationship(
        "CreditHistory",
        back_populates="seller",
        foreign_keys="CreditHistory.seller_id",
        uselist=False,
    )
...
class LinkedWallet(Base):
    ...
    # Constraints
    __table_args__ = (
        UniqueConstraint("wallet_address", "user_id", name="_wallet_user_uc"),
    )
...
class Invoice(Base):
    ...
    supply = Column(
        Integer, default=1
    )  # 1 for whole invoice, N for N fractional shares
    token_id = Column(
        String, nullable=True, index=True
    )  # ERC1155 token ID from smart contract
...
class CreditHistory(Base):
    ...
    seller = relationship(
        "User", foreign_keys=[seller_id], back_populates="credit_history"
    )
...
class KycSubmission(Base):
    ...
    status = Column(
        Enum(KycStatus), nullable=False, default=KycStatus.pending, index=True
    )
...
class CreditEvent(Base):
    ...
    )  # ON_TIME_PAYMENT,LATE_30,LATE_60,LATE_90
...
class AuctionBid(Base):
    ...
    )  # active,canceled,winning,outbid
    created_at = Column(
        DateTime(timezone=True), nullable=False, server_default=func.now(), index=True
    )
...
class MarketplaceListing(Base):
    ...
    listing_type = Column(
        String, nullable=False, default="fixed", index=True
    )  # fixed,auction,fractional
    status = Column(
        String, nullable=False, default="active", index=True
    )  # active,paused,sold,canceled
    ask_price = Column(Float, nullable=True)
    share_price = Column(Float, nullable=True)
...
class MarketplaceAuction(Base):
    ...
    invoice_id = Column(Integer, ForeignKey("invoices.id"), index=True, nullable=False)
    listing_id = Column(
        Integer, ForeignKey("marketplace_listings.id"), index=True, nullable=True
    )
    seller_id = Column(Integer, ForeignKey("users.id"), index=True, nullable=False)
    status = Column(
