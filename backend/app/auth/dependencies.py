...
def require_seller(current_user: User = Depends(get_current_user)):
    ...
    if current_user.role != UserRole.SELLER:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Only sellers can access this"
        )
    return current_user
...
def get_current_admin_or_investor(
...
async def verify_wallet_ownership(
    ...
    wallet = (
        ...
        .filter_by(user_id=current_user.id, wallet_address=addr, is_active=True)
        .first()
    )
