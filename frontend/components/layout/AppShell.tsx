...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const isKycGatedPath = pathname.startsWith("/sme") || pathname.startsWith("/investor");
  const isKycAlwaysAllowedPath =
    pathname.startsWith("/sme/dashboard") || pathname.startsWith("/investor/marketplace");

  const role = String(user?.role || "").toLowerCase();
  const isOwnerRoute =
    (pathname.startsWith("/sme") && (role === "seller" || role === "sme")) ||
    (pathname.startsWith("/investor") && role === "investor");
  const isKycVerifiedForAccess = profile?.kyc?.status === "approved" || profile?.kyc?.status === "review";
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <div className="w-full max-w-xl rounded-xl border bg-card p-8 text-center shadow-sm">
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              You cannot access this page until your KYC is submitted and under review or approved.
            </p>
            <div className="mt-6">
              <Link
                href="/profile"
