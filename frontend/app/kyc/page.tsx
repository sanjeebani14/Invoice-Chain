...
function statusLabel(kyc: KycSubmissionOut | null) {
  ...
  if (kyc.status === "approved") return { text: "Approved", tone: "text-green-600" };
  if (kyc.status === "rejected") return { text: "Rejected", tone: "text-red-600" };
...
export default function KycPage() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const message = err?.response?.data?.detail ?? "KYC upload failed";
      toast.error(message);
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="mt-6 rounded-2xl border border-border bg-card p-6">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className={`mt-1 text-lg font-medium ${status.tone}`}>{status.text}</p>
            </div>
            {(loading || authLoading) && (
          ...
          <div className="mt-6 space-y-2">
            <p className="text-sm text-muted-foreground">
              Upload your <span className="font-medium text-foreground">PAN</span> (PDF/JPG/PNG, max 10MB).
            </p>
            <Input
              type="file"
